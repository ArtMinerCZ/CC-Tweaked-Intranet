...
---returns parsed page's width and height
---@param page table
---@return integer width
---@return integer height
function mod.get_page_size(page)
    local max_width = 0
    local current_width = 0

    for i = 1, page.content.len do
        local element = page.content[i]
        ...
        elseif type(element) == "string" then
            current_width = current_width + #element
        end
    end
    ...
    local width = math.max(1, max_width)
    local height = math.max(1, page.line_count or 1)
